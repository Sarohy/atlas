...
class RsiIndicator(BaseModel):
    ...
    score: int = Field(ge=0, le=20, description="Momentum score contribution (0-20).")
    max_score: int = Field(default=20, description="Maximum possible score for this indicator.")
...
class MaAlignmentIndicator(BaseModel):
    ...
    label: str = Field(
        description=(
            "Alignment label: FULL_BULL | BULL | MIXED | BEAR | FULL_BEAR | INSUFFICIENT_DATA"
        )
    )
    score: int = Field(ge=0, le=20, description="Momentum score contribution (0-20).")
    max_score: int = Field(default=20)
...
class PerformanceIndicator(BaseModel):
    ...
    score_1m: int = Field(ge=0, le=10, description="1-month sub-score (0-10).")
    score_6m: int = Field(ge=0, le=10, description="6-month sub-score (0-10).")
    score: int = Field(ge=0, le=20, description="Combined performance score (0-20).")
    max_score: int = Field(default=20)
...
class SectorMomentumIndicator(BaseModel):
    ...
    ticker_perf_3m: float = Field(description="Ticker 3-month total return (%).")
    sector_perf_3m: float = Field(description="Sector ETF 3-month total return (%).")
    relative_perf_3m: float = Field(description="Ticker minus sector 3-month return (%).")
    score: int = Field(ge=0, le=20, description="Momentum score contribution (0-20).")
    max_score: int = Field(default=20)
